...
def view_Artist(request , author_user=None , tag=None):
    ...
    context = {"posts":posts , "Artworks":Artworks }
...
def single_page(request , pid):
    ...
    Art_of_artist = Artwork.objects.filter(poblished_date__lte=time_now , status= 1 , artist=post.title )
    if not post.login_require:
        ...
        def addview(post):
            ...
            return post.save() 
        addview(post)
        context = {"post":post , "works":Art_of_artist} 
